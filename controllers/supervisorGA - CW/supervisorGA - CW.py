...
class SupervisorGA:
    ...
    def evaluate_genotype(self,genotype,generation):
        ...
        while currentInteraction < numberofInteractionLoops:
            ...
            # Check for Reward and add it to the fitness value here
            # Add your code here
            [tx,ty,tz]=self.robot_node.getPosition()#get the current robot positions for calculating the 
                                                    #distance between the robot and goal 
                                                    #in this case goal is x=-0.36, y=-0.155
            dist=np.sqrt((-0.36-tx)**2+(-0.155-tz)**2)
            dist=(dist*1.7)**3
            if(dist>1):
                dist=1
