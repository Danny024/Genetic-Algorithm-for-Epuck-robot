...
class Controller:
    ...
    def calculate_fitness(self):
       ...
       ###All fitness values are normalized to the range [0,1] for easier fitness calculations
       ###Forward fitness improves the faster the robot drive forwards this is required in order the push
       ### to the goal.
        ...
        if (forwardFitness<0):
            forwardFitness=0
           #print(self.velocity_left,self.velocity_right)          
        ...
        if(np.max(self.inputs[3:11])):
            ...
            # The maximum value from all proximity sensors from ps1 to ps8 is taken
        ...
        combinedFitness = (forwardFitness + (2*avoidCollisionFitness) + spinningFitness)/4
        #combined fitness is the mean of all fitness with collision fitness having more priority
        #it lies in range[0,1]
        #print("Fitness \n    forward {} collision {} spin {}".format(forwardFitness,(2*avoidCollisionFitness),spinningFitness))
